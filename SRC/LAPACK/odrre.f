*> \brief \b ODRRE given the tridiagonal matrix T, sets small off-diagonal elements to zero and for each unreduced block Ti, finds base representations and eigenvalues.
*
*  =========== DOCUMENTATION ===========
*
...
*  ===========
*
*       SUBROUTINE ODRRE( RANGE, N, VL, VU, IL, IU, D, E, E2,
...
*     Compute splitting points
      CALL ODRRA( N, D, E, E2, SPLTOL, SPDIAM,
     $                    NSPLIT, ISPLIT, IINFO )
...
*        ODRRD needs a WORK of size 4*N, IWORK of size 3*N
         CALL ODRRD( RANGE, 'B', N, VL, VU, IL, IU, GERS,
...
*        Compute the negcount at the 1/4 and 3/4 points
         IF(MB.GT.1) THEN
            CALL ODRRC( 'T', IN, S1, S2, D(IBEGIN),
...
*        Store the shift.
         E( IEND ) = SIGMA
*        Store D and L.
         CALL ODCPY( IN, WORK, 1, D( IBEGIN ), 1 )
         CALL ODCPY( IN-1, WORK( IN+1 ), 1, E( IBEGIN ), 1 )
...
*           use bisection to find EV from INDL to INDU
            CALL ODRRB(IN, D(IBEGIN), WORK(IBEGIN),
